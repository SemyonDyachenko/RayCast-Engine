...
void SceneHierarchy::Render(EditorScene& editorScene)
{
    ...
    if (ImGui::TreeNodeEx((void*)(uint64_t)(uint32_t)88543223, camera_flags, "Editor Camera")) {

        if (ImGui::IsItemClicked()) {
            HasSelected = false;
            m_SelectedEntity = {};
            m_EditorCameraSelected = true;
        }
    ...
    for (auto& entity : m_Scene->GetManager().GetEntities()) {
        if (entity) {
            ...
            /*    if (ImGui::MenuItem("Copy"))
                {
                    ClipboardUtils::AddEntityToClipboard(&(*entity));
                }*/
            ...
            if (entityDeleted) {
                if (entity->HasComponent<RigidBodyComponent>()) {
                    m_Scene->GetPhysicsWorld().RemoveRigidBody(&entity->GetComponent<RigidBodyComponent>().rigidbody);
                }
    ...
    if (ImGui::BeginPopupContextWindow(0, 1, false))
    {
       ...
       /* if (ClipboardUtils::HasEntity())
        {
            if (ImGui::MenuItem("Paste")) {

                auto* entityFromClipboard = ClipboardUtils::GetEntityFromClipboard();
                auto createdEntity = m_Scene->CreateEntity(m_Scene->GetEntitiesCount(), entityFromClipboard->GetComponent<TagComponent>().tag);
                auto& entity = m_Scene->GetEntity(createdEntity.GetId());
                entity->GetComponent<TransformComponent>().Position = entityFromClipboard->GetComponent<TransformComponent>().Position;
                entity->GetComponent<TransformComponent>().Rotation = entityFromClipboard->GetComponent<TransformComponent>().Rotation;
                entity->GetComponent<TransformComponent>().Scale = entityFromClipboard->GetComponent<TransformComponent>().Scale;

                
                std::vector<Vertex> raw = OBJLoader::loadObjModel(entityFromClipboard->GetComponent<MeshComponent>().mesh.GetFilename());
                Mesh* mesh = new Mesh(raw.data(), raw.size(), 0, 0, entityFromClipboard->GetComponent<MeshComponent>().mesh.GetFilename());
                glm::vec3 solidColor = entityFromClipboard->GetComponent<MeshComponent>().mesh.GetSolidColor();
            	entity->AddComponent<MeshComponent>(*mesh, solidColor);

...
Mesh* ObjectAdditor::CreateMesh(DefaultObjects object)
{
	std::string path;
...
void ObjectAdditor::AddEntity(std::string meshPath, std::string name, EditorScene& target, GuiConsole& console, glm::vec3 position)
{
	auto& entity = target.CreateEntity(target.GetEntitiesCount(), name);
	std::vector<Vertex> rawModel = OBJLoader::loadObjModel(meshPath);
	Mesh* mesh = new Mesh(rawModel.data(), rawModel.size(), NULL, 0,meshPath);	glm::vec3 defaultColor = { 0.8,0.8,0.8 };

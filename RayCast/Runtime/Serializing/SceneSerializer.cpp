...
static void SerializeObject(YAML::Emitter& out, Entity entity)
{
	...
	if (entity.HasComponent<PBRMaterialComponent>()) {
		...
		if (material.material.IsActive()) {
			...
			out << YAML::Key << "Material File" << YAML::Value << material.material.GetPath();
	...
	if (entity.HasComponent<RigidBodyComponent>()) {
		...
		out << YAML::Key << "Position" << YAML::Value << PhysicUtils::BtVec3ToGLM(rigidBody.rigidbody.GetWorldTransform().getOrigin());
		out << YAML::Key << "Rotation" << YAML::Value << glm::eulerAngles(PhysicUtils::BtQuatToGLM(rigidBody.rigidbody.GetWorldTransform().getRotation()));
		...
		out << YAML::Key << "TakesGravity" << YAML::Value << rigidBody.rigidbody.TakesGravity();
			...
			for (auto entity : entites) {
				...
				if (resultEntity) {
					...
					if (materialComponent) {
						...
						Texture* diffuseTexture = new Texture(diffuseMapPath.c_str());
						Texture* specularTexture = new Texture(specularMapPath.c_str());
					...
					if (pbrMaterial) {
						auto MaterialPath = pbrMaterial["Material File"].as<std::string>();
